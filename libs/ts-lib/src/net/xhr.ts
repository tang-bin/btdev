...
class XHR extends Model {
    public token = '';
    public API_HOST = '';
    public API_BASE = '/phoenix/rest/h5';
    public headers: any = {
    ...
    public request(
        ...
        headers?: any,
        data?: any,
        handlers?: { [name: string]: progressHandler }
    ): Promise<any> {
        // method
        ...
        return new Promise((resolve, reject) => {
            ...
            req.addEventListener(
                ...
                (e: ProgressEvent<XMLHttpRequestEventTarget>) => {
                    handlers && handlers['progress']?.call(null, e);
                }
            );
            req.addEventListener(
                'load',
                (e: ProgressEvent<XMLHttpRequestEventTarget>) => {
                    handlers && handlers['load']?.call(null, e);
                    if (req.status !== 200) {
            ...
            req.addEventListener(
                'error',
                (e: ProgressEvent<XMLHttpRequestEventTarget>) => {
                    handlers && handlers['error']?.call(null, e);
                    reject(`XHR Error ${req.status}: ${req.statusText}`);
                }
            );
            req.addEventListener(
                'abort',
                (e: ProgressEvent<XMLHttpRequestEventTarget>) => {
                    handlers && handlers['abort']?.call(null, e);
                    reject(`XHR Error ${req.status}: ${req.statusText}`);
                }
            );
    ...
    private _buildURL(api: string, argv?: any): string {
        ...
        let url =
            api.charAt(0) === '/'
                ? api
                : this.API_HOST + this.API_BASE + '/' + api;
        // add token
        argv = argv || {};
        argv.s = this.token;
        // add timestamp
